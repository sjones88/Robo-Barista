...
public class RoboBarista{
    ...
    private void runner() {
        ...
        if ( orderNums.contains(request) ) {
            String order = menu.get(Integer.parseInt(request) - 1);
        ...
        } else if ( request == String.valueOf('q') ) {
            return;
        } else if ( request == String.valueOf('r') ) {
            ...
            printInventory();
            printMenu();
            runner();/* package codechef; // don't place package name! */
...
class RoboBarista{
    ...
    private void runner() {
        ...
        if ( orderNums.contains(request) ) {
            ...
            return;
        } else if ( request == validInputs.get(1) ) {
            for ( String item : inventoryUnits.keySet() ) {
                inventoryUnits.put(item, 10.0);
            }
            printInventory();
